...
典型的高斯过程回归方法是为了对于任意测试数据$$\mathbf{x}^*\in X$$估计预测分布$$p\left( { y |{{\mathbf{x}}^*}} )$$，基于高斯概率假设，可以得到条件分布：<br>

```math
\hat p\left( {y\left| {{{\bf{x}}^*}} \right.,X,Y} \right)\~N\left( {f\left( {{{\bf{x}}^*}} \right),g\left( {{{\bf{x}}^*}} \right)} \right)
```
