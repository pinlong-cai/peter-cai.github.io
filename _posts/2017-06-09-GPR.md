...
定义$$\mathbf{K}_{X\mathbf{x}}=[k(\mathbf{x}_1,\mathbf{x}),\dots,k(\mathbf{x}_N,\mathbf{x})]^T\in \Re^N$$,典型的高斯过程回归方法是为了对于任意测试数据$$\mathbf{x}^*\in X$$估计预测分布$$p(y |{\mathbf{x}^*} )$$，基于高斯概率假设，可以得到条件分布：<br>
